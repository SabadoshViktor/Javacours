...
public class Rand {
	...
	public String getRandomString(int minLength, int maxLength) {
		...
		for (int i = 0; i < len; i++) {
			...
			// We need byte value, cause casting from int to char is impossible
...
//		long min = from.toEpochDay();  TO UNDERSTAND
//		long max = to.toEpochDay();
//		long randomDay = ThreadLocalRandom.current().nextLong(min, max); //Random day in period 
